...
int main(int argc, char** argv) {
  if(argc<4) {
    std::cerr << "Usage: input_cams_file input_points_file input_cfy_file" << std::endl;
    return 0;
  }
  ...
  cv::TermCriteria criteria(cv::TermCriteria::MAX_ITER+cv::TermCriteria::EPS, 150, 1e-10);
  ...
  // Leer archivo fy (focales en y)
  //std::ifstream file("/home/esther/Escritorio/cvsba-1.0.0/utils/cfy08.txt"); // actualizar archivo
  std::ifstream file(argv[3]); // actualizar archivo  
  std::string line;
  
  int cont = 0;
  if(file.is_open()){
  	while(std::getline(file,line)){
	  	cameraMatrix[cont].ptr<double>(0)[4]=std::stod(line);
		cont++;  	
  	}
  	file.close();
  }
  ...
  // Guardar nuevos puntos en txt 
  
  ofstream outfile("/home/esther/Escritorio/TFM/process_all/cnewpts.txt");
  if(outfile.is_open()){
  	...
  	outfile.close();
  	cout<<"-Nuevos puntos 3D guardados-"<<endl;
  }
  else{
  	cout<<"No se han podido guardar los nuevos puntos 3D"<<endl;
  }

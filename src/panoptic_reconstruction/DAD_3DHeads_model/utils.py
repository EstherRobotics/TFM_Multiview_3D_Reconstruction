...
def indices_reweighing(weights_and_indices: Dict[str, Any]) -> Tuple[List, List]:
    ...
    '''with open('indices1.json', 'w') as f:
                    json.dump(np.array(indices[0]).tolist(), f)
            
                with open('indices2.json', 'w') as f:
                    json.dump(np.array(indices[1]).tolist(), f)
            
                with open('indices3.json', 'w') as f:
                    json.dump(np.array(indices[2]).tolist(), f)
            
            
                with open('weights.json', 'w') as f:
                    json.dump(np.array(weights).tolist(), f)'''
